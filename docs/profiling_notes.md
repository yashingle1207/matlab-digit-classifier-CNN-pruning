...
The before screenshot represents the profiling state before the pruning-focused optimization workflow. The after screenshot represents the profiling state after pruning and quantization as documented in the coursework materials.

These screenshots are included as supporting evidence from the original project. They are not regenerated by the current repository.
...
- The screenshots and metrics are preserved from the original group coursework materials.
- The repository cleanup did not re-run Flash/RAM profiling or energy profiling.
- The repository does not include a complete embedded firmware project or hardware reproduction script.
- Tool versions, board configuration, measurement setup, and workload details are not fully captured in the current repository.
- The metrics should be treated as documented project results, not independently verified benchmark claims.
- No additional deployment steps, Simulink workflow, latency numbers, or hardware measurements are claimed here.
